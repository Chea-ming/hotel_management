...
.btn--action{
  ...
  box-shadow: 0 10px #f9ca24;
                    ...
                    <div class="card-header">
                        <h2 style="font-family: 'Glyphicons Halflings';"><b>House Keeping Information</b></h2>
                    </div>
                    <div class="card-body">
                        <a href="{{ url('/housekeepings/create') }}" style="padding:10px; background-color: #63a91f;
background-image: linear-gradient(314deg, #63a91f 0%, #1a512e 74%);"class="btn btn-success btn-sm" title="Add New Entry"> New Registration </a>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                        <tr>
                        @foreach($housekeepings as $item)

                            <!-- <td><b>{{ $loop->iteration }}</b></td> -->
                            <td><b>{{ $item->HousekeepingID }}</b></td>
                            ...
                            <td>
                                <a href="{{ url('/housekeepings/' . $item->HousekeepingID) }}" title="View Details">
                                    <button style="margin-bottom:2px;" class="btn btn-info btn-sm"><i class="fas fa-eye" aria-hidden="true"></i>  View</button>
                                </a>
                                <a href="{{ url('/housekeepings/' . $item->HousekeepingID . '/edit') }}" title="Edit Details">
                                    <button style="margin-bottom:2px;" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i> Edit</button>
                                </a>
                                <form method="POST" action="{{ url('/housekeepings' . '/' . $item->HousekeepingID) }}" accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
