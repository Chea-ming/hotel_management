...
.btn--action{
  ...
  box-shadow: 0 10px #f9ca24;
                    ...
                    <div class="card-header">
                        <h2 style="font-family: 'Glyphicons Halflings';"><b>Events Information</b></h2>
                    </div>
                    <div class="card-body">
                        <a href="{{ url('/events/create') }}" style="padding:10px; background-color: #63a91f;
background-image: linear-gradient(314deg, #63a91f 0%, #1a512e 74%);"class="btn btn-success btn-sm" title="Add New Entry"> New Registration </a>
                    ...
                    <table class="table">
                        ...
                        @foreach($events as $item)
                            ...
                            <td><b>{{ $item->EventID }}</b></td>
                            <td>{{ $item->EventName ?? '-' }}</td>
                            <td>{{ $item->EventDateTime ?? '-' }}</td>
                            <td>{{ $item->Venue ?? '-' }}</td>
                            <td>{{ $item->GuestList ?? '-' }}</td>
                            <td>{{ $item->EventOrganizer ?? '-' }}</td>


                            <td>
                                <a href="{{ url('/events/' . $item->EventID) }}" title="View Details">
                                    <button style="margin-bottom:2px;" class="btn btn-info btn-sm"><i class="fas fa-eye" aria-hidden="true"></i>  View</button>
                                </a>
                                <a href="{{ url('/events/' . $item->EventID . '/edit') }}" title="Edit Details">
                                    <button style="margin-bottom:2px;" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i> Edit</button>
                                </a>
                                <form method="POST" action="{{ url('/events' . '/' . $item->EventID) }}" accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
